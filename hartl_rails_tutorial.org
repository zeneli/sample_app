...
After adding the [[http://api.rubyonrails.org/classes/ActiveModel/SecurePassword/ClassMethods.html#method-i-has_secure_password][has_secure_password]] to the User model and running the
tests, we see RED on the tests that assert validity of the =@user=
from setup. All other tests using =assert_not= remain passing since
