...
w/ the (slightly) more expressive
...
model w/ a table in the database. \\
We add a second such resource: user microposts, which are short
messages associated w/ a particular user. We will make a full strength
version of the sketch from section 2.3 by constructing a Micropost
data model, associating it w/ the User model using the =has_many= and
