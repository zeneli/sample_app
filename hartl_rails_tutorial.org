...
Method: require each user to have a password (with password
...
the tests create a =@user= variable without these attributes. \\
...
*Conclusion*. \\
Starting from scratch, we have a User model with name, email, and
password attributes with enforcing validations). We have the ability to
securely authenicate users using a given password (all in twelve lines
of code). \\
In ch7, we make a working signup form to create new users with a page
...
- app creates a new user and saves its attributes to the database
  (cf. 7.4)
- the app renders a profile page with the newly created user information
We begin with a page /showing/ users (as a first step towards our REST
...
classes in place, we can style the profile page with scss inside the
...
: ...
: <% end %>
The =form_for= take a block with one variable (viz. =|f|= for
"form"). When the =f= object is called with a method corresponding to an
...
call), Rails knows to construct a form with the =post= method. 
#+BEGIN_SRC ruby
<%= f.label :name %>
<%= f.text_field %>
# produces the HTML
...
- we then /redirect/ the browser to show the users profile (with a
  greeting)
...
to make signup secure, and replace the default webserver with one
...
to log in and out of the app (with optional 'remember me'
...
be able to update the site header with login/logout links and a profile
link. \\
...
associated with the user. Ch14 allows the current user to follow other
...
- =DELETE destroy= logging out is handled by sending a DELETE request to the =destroy=
  action
We begin by generating a Sessions controllers with a =new= action:
...
- then lay the foundations for successful login by evaluating each
  ...
  combination
We start with a minimalist =create= action for the Sessions controller
...
./app/views/users/new.html.erb
  ...
  <% end %>
#+END_SRC
The errors are associated with a particular Active Record object, but
...
log the user in with a temporary session cookie that expires
...
The =log_in= method. \\
Logging a user in is simple with the help of the =session= method
...
temp sessions via =session= method, not with sessions via =cookie=
...
the behavior described above. As you become mroe familiar with the
...
pass). Now we're ready to start filling in the empty file with custom
fixtures of our own. \\
...
user in, we also have to include a valid password to compare with the
...
with fixtures, and adding a =password= attribute cuases Rails to
...
password =password=. \\
Having created a fixture with a valid user, we can retrieve it inside a
test as follows:
: user = users(:foo)
Here =users= corresponds to the fixture filename =user.yml=, while the
symbol =:foo= references user with the key shown. With the fixture user
...
*Edit form*. \\
We begin with the edit form which consists of name, email, and
...
associated with it; dealing with the details is covered below. \\
WITH the =@user= instance variable, the edit page should render properly
the 'Name' and 'Email' fields with the corresponding =@user=
attributes. \\
Since web browsers can't natively send =PATCH= requests (as required
by the REST conventions, Rails fakes it with a =POST= request and a
...
=edit_user_path= named route, together with the =current_user= helper
method (see below).
#+BEGIN_SRC ruby
...
As we get more comfortable with testing, you might find that it's useful
...
We'll test for the correct behavior of updating users with the following
behavior in mind:
...
class UserEditTest < ActionDispatch::IntegrationTest
  ...
  test "successful edit" do
    ...
    assert_template 
#+END_SR
** [[https://www.railstutorial.org/book/updating_and_deleting_users#sec-authorization][Authorization]]
...
hole like this is #1. So the code without the our security model should
definitely be RED. \\
...
the =edit= and =update= actions with the right kinds of requests and
...
users fixture (that is add user =bar=). By using the =log_in_as= method, we
...
second method called =correct_user=, together with a before filter
...
As some final refactoring: we adopt a common convention and define a
=current_user?= boolean method for use in the =correct_user= before
filter (defined in the Sessions helper). We use this method to replace
code like:
 : unless @user == current_user
with the (slightly) more expressive
 : unless current_user?(@user)
#+BEGIN_SRC ruby
test/controllers/user_controller.rb
...
*Friendly Forwarding*. \\
Blemish: when users try to access a protected page, they are currently
redirected to their profile page regardless of where they where trying
to go. 
So if a non-logged-in user tries to visit the edit page, after logging
in the user will be redirected to /users/1 instead of
/users/1/edit. It would be much friendlier to redirect them to their
intended destination instead. \\
* User Microposts
In th course of developing the core sample app, we've encountered four
resources--users, session, account activations, and password
resets--but only the first of these is backed by an Active Record
model with a table in the database. \\
We add a second such resource: user microposts, which are short
messages associated with a particular user. We will make a full strength
version of the sketch from section 2.3 by constructing a Micropost
data model, associating it with the User model using the =has_many= and
=belongs_to= methods, and then making the forms and partials needed to
manipulate and display the results (including uploaded images). In
ch14, we'll complete our tiny Twitter clone by adding the notion of
following usres in order to receive a feed of their microposts. \\
