...
The session resource will use cookies, and much of the work involved
in login comes from building this cookie-based authentication
machinery. \\
...
- logging out is handled by sending a DELETE request to the =destroy=
  action
...
Including =new= actually generates /views/ as well, which is why we
don't include actions like =create= and =destroy= that don't
correspond to views. \\
Unlike the Users resource, which used the special =resources= method
to obtain a full suite of RESTful routes automatically, the Sessions
resource will use only name routes, handling GET and POST requests w/
the =login= route and DELETE request w/ the =logout= route. 
